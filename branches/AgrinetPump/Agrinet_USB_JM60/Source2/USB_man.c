...
#define USBSendSatellitare_en   _USB_flags1.BIT.B3


extern byte SuspendAllUSB;
byte state_USBSend;

void InviaSatellitare(void){
  USBSendSatellitare_en = TRUE;
}
 ...
 *    USB_SimpleSend
 * Description:
 *    _toSend è l'array da inviare
 *    _len    è la lunghezza dell'array
 *
 *****************************************************************************/
void USB_SimpleSend(byte *_toSend, byte _len)
{
  byte cnt=0;
  ...
  switch (state_USBSend) {
    ...
    case 1:
      if (allarme_in_lettura == operazione_effettuata) {        
        USB_PktSend(buffer_USB,byte_tabella, REQ_HELLO);          
 ...
 *    USB_SendSatellitare
 * Description:
 *      Send valori per satellitare
 *****************************************************************************/
void USB_SendSatellitare(void)
{  
  switch (state_USBSend) {
    case 0:        
      allarme_in_lettura = lettura_satellitare;
      state_USBSend = 1;
    break;
