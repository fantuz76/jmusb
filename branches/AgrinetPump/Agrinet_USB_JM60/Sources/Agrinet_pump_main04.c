...
menu_principale[2][84][17]={//Menu' principale: elenco delle funzioni di taratura
"1) Number       ",//0 italiano
"Password:       ",
"2)Single/Three  ",//1
"                ",
...
attesa_per_emergenza_differenziale=1000,//ms
tot_righe_menu=84,
/*
...
2,    //ritardo_funzionamento_dopo_emergenza,//min
...
40,   //ritardo_stop_funzionemento_a_secco //s
10,   //ritardo_riaccensione_mandata_chiusa //s
2,    //ritardo_riaccensione_funzionamento_a_secco //min
...
4,    //costante_tau_salita_temperatura s
...
5,    //ritardo_stop_funzionemento_a_secco //s
...
240,  //timeout_protezione_tensione s
...
999,  //ritardo_funzionamento_dopo_emergenza,//min
...
999,  //ritardo_stop_funzionemento_a_secco //s
14400,//ritardo_riaccensione_mandata_chiusa //s
999,  //ritardo_riaccensione_funzionamento_a_secco //min
...
;pressione=(DAC.sensore_pressione-491)*set.portata_sensore_pressione/fattore_portata;
 ...
 case 16:
  ...
  if((set.monofase_trifase)&&(set.monofase_trifase)&&(relay_alimentazione)) cosfi[0]=calcolo_del_fattore_di_potenza(attiva,reattiva); else cosfi[0]=0;//cosfi*100
 ...
 case 17:
  ...
  if((set.monofase_trifase)&&(set.monofase_trifase)&&(relay_alimentazione)) cosfi[1]=calcolo_del_fattore_di_potenza(attiva,reattiva); else cosfi[1]=0;//cosfi*100
 ...
 case 18:
  ...
  if((set.monofase_trifase)&&(set.monofase_trifase)&&(relay_alimentazione)) cosfi[2]=calcolo_del_fattore_di_potenza(attiva,reattiva); else cosfi[2]=0;//cosfi*100
...
if(toggle_func)//lettura e modifica delle funzioni
 ...
 if(salita_stop)
  ...
  salita_start=0;
  start=0;
  if(cursore_menu==35) //presentazione dell'elenco emergenze
   {
 ...
 if(toggle_stop==0)//modifica della funzione
  ...
  if(salita_meno)
   ...
   if(numero_ingresso==chiave_ingresso)
    {
    if(cursore_menu<35) cursore_menu++; else cursore_menu=1;
    } 
   else if(numero_ingresso==chiave_numero_serie)
    {
    if(cursore_menu<41) cursore_menu++; else cursore_menu=1;
    } 
   else// nessuna chiave
    {
    if(cursore_menu==36) cursore_menu=0;
    else if(cursore_menu<4) cursore_menu++;
    else cursore_menu=36;
    }
   ...
   if(numero_ingresso==chiave_ingresso)
    {
    if(cursore_menu>1) cursore_menu--; else cursore_menu=35;
    } 
   else if(numero_ingresso==chiave_numero_serie)
    {
    if(cursore_menu>1) cursore_menu--; else cursore_menu=41;
    } 
   else// nessuna chiave
    {
    if(cursore_menu==36) cursore_menu=4;
  ...
  switch(cursore_menu)
   ...
   case 24: presenta_unsigned(set.ritardo_riaccensione_mandata_chiusa,0,1,9,5); break;
 ...
 //case 35: storico allarmi
   ...
   case 37: presenta_unsigned(set.numero_serie,0,1,7,5); break;
   case 38: presenta_scritta((char*)&attivazione_comando_reset,1,0,1,1,0,16); break;
   case 39: presenta_unsigned(set.calibrazione_I1,0,0,12,3); break;
   case 40: presenta_unsigned(set.calibrazione_I3,0,0,12,3); break;
   case 41: presenta_unsigned(set.calibrazione_I2,0,0,12,3); break;
   }
  }
 //if(toggle_stop) modifica del valore 
 else switch(cursore_menu)
  {
  case 0:  //abilitazione delle funzioni
   {
...
//   modifica_unsigned((unsigned int*)&set.monofase_trifase,0,1,0);
  ...
  case 10:
   {
   modifica_unsigned((unsigned int*)&set.timeout_protezione_tensione,2,240,0);
  ...
  case 21:
   {
   modifica_unsigned((unsigned int*)&set.ritardo_funzionamento_dopo_emergenza,1,999,0);
  ...
  case 24:
   {
   modifica_unsigned((unsigned int*)&set.ritardo_riaccensione_mandata_chiusa,3,14400,1);
   presenta_unsigned(set.ritardo_riaccensione_mandata_chiusa,0,1,9,5);
   } break;
  case 25:
   {
  ...
  case 26:
   {
   modifica_unsigned((unsigned int*)&set.ritardo_stop_funzionemento_a_secco,5,999,0);
   presenta_unsigned(set.ritardo_stop_funzionemento_a_secco,0,1,11,3);
   } break;
  case 27:
   {
   modifica_unsigned((unsigned int*)&set.ritardo_riaccensione_funzionamento_a_secco,2,999,0);
   presenta_unsigned(set.ritardo_riaccensione_mandata_chiusa,0,1,10,3);
   } break;
  case 28:
   {
   ...
   &&(remoto==filtro_pulsanti))
    ...
    ((set.abilita_sensore_pressione)&&(pressione<set.pressione_accensione)&&(pressione>emergenza_sensore_pressione)))
     ...
     ((set.monofase_trifase)&&(dissimmetria<dissimmetria_tollerata)))
      ...
      timer_attesa_differenziale=attesa_per_emergenza_differenziale;
      if(set.motore_on==2)//misura energia
       {
       toggle_stop=0;
       toggle_func=1;
       cursore_menu=36;
...
if(toggle_func==0)//marcia normale
 ...
 //entro le funzioni di taratura lettura corrente e misura energia 
else if((salita_start)&&(
(cursore_menu==36) //possibile partenza in misura energia
||(cursore_menu==39)||(cursore_menu==40)||(cursore_menu==41))) //partenza possibile in taratura correnti
 {
 salita_start=0;
 timer_commuta_presentazione=0;
 if((relay_alimentazione==0)&&(remoto==filtro_pulsanti))
  {
  if(cursore_menu==36) set.motore_on=2;//misura energia
   else set.motore_on=1;
  segnalazione_=1;
  timer_rele_avviamento=100;
  relay_alimentazione=1;
  prima_segnalazione=0;
...
else if((salita_stop)&&(cursore_menu==36))
 {
 salita_stop=0;
 toggle_stop=0;
 segnalazione_=0;
