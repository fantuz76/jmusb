...
lunghezza_trasmissione_satellitare=29;//29+67
const char
...
energia[3],//51 //KWh
totalizzatore_litri[3],//54
timer_relay_avviamento;//57

unsigned long
scala_sensore_di_flusso,//litri*100/impulso   1-100000
volume_massimo,//litri   1-1000000
timer_riavviamento;//58
...
//per ogni secondo
 ...
 LDA set.timer_relay_avviamento:1
 SUB #1
 STA set.timer_relay_avviamento:1
 LDA set.timer_relay_avviamento
 SBC #0
 STA set.timer_relay_avviamento

per_timer_riavviamento: 
 LDHX set.timer_riavviamento
 BNE al_decremento
 LDHX set.timer_riavviamento:2
 BEQ per_energia
al_decremento:
 LDA set.timer_riavviamento:3
 SUB #1
 STA set.timer_riavviamento:3
 LDA set.timer_riavviamento:2
 SBC #0
 STA set.timer_riavviamento:2
 LDA set.timer_riavviamento:1
 SBC #0
 STA set.timer_riavviamento:1
 LDA set.timer_riavviamento
 SBC #0
 STA set.timer_riavviamento
...
// if(set.abilita_contetore_down) if(volume_decrementato>0) {volume_decrementato -=incremento_volume; if(volume_decrementato==0) arresto_per_volume=1; }
 ...
 LDHX volume_decrementato
 BNE per_dec_volume
 LDHX volume_decrementato:2
 BEQ calcolo_della_portata
 CPHX #2
 BCC per_dec_volume
...
per_dec_volume: 
 ...
 LDA volume_decrementato:3
 SBC incremento_volume:1
 STA volume_decrementato:3
 LDA volume_decrementato:2
 SBC incremento_volume
 STA volume_decrementato:2
 LDA volume_decrementato:1
 SBC #0
 STA volume_decrementato:1
 LDA volume_decrementato
 SBC #0
 STA volume_decrementato
 
calcolo_della_portata:
//portata[l/min] = scala_sensore_di_flusso[litri*100/impulso]/100 *60*1000000/10.66667/delta_timer_flusso;
//portata[l/min] = scala_sensore_di_flusso[litri*100/impulso]*56150/delta_timer_flusso;
...
assegna_prime_limitazioni:
 ...
 LDHX #0
 STHX set.timer_riavviamento
 STHX set.timer_riavviamento:2
...
// 1 byte = temperatura  0-255 °C
 ...
 LDA set.segnalazione_;//tipo intervento:
 STA buffer_fault
...
rotazione_4_Volte: 
 ...
 LDHX set.potenza_nominale //KW*100
 STHX data
per_media_pressione:
 LDHX pressione
 STHX data:2
...
if(salita_esc)
 ...
 //comando di salvataggio in eeprom
   ...
   {
   toggle_esc=1;
   presenta_menu((unsigned char*)&exit_program,0,2,0);
...
if(toggle_func)//lettura e modifica delle funzioni
  ...
  if(timer_aggiorna_menu==0) //attesa per presentazione accesso abilitato
   ...
   if(salita_meno)
    ...
    if(numero_ingresso==chiave_ingresso2)
     ...
     {
     if(cursore_menu==0) cursore_menu=1;
     else if(cursore_menu==1) cursore_menu=7;
    ...
    if(numero_ingresso==chiave_ingresso2)
     ...
     {
     if(cursore_menu==7) cursore_menu=1;
     else if(cursore_menu==1) cursore_menu=0;
     if(cursore_menu==1) prepara_chiave=1; else prepara_chiave=0;
     } 
    salita_piu=0;
    }
   if(cursore_menu==0) //potenza motore
    ...
    scrivi_display=0;
    presenta_scritta((unsigned char*)&presentazione_iniziale,posizione_scritta,0,0,1,0,16);
  ...
  */
    ...
    case 7://sensore_di_flusso
     ...
     presenta_menu((unsigned char*)&sensore_di_flusso,0,12,cursore_sottomenu<<1);
     switch(cursore_sottomenu)
      {
      case 0:
       {
       if(set.abilita_sensore_flusso) presenta_scritta((char*)&lettura_allarmi,22,0,24,1,8,7);
       else
        {
        presenta_scritta((char*)&lettura_allarmi,5,0,24,1,8,7);
      ...
      case 4:
       {
       if(set.abilita_contatore_down) presenta_scritta((char*)&lettura_allarmi,22,0,24,1,8,7); else presenta_scritta((char*)&lettura_allarmi,5,0,24,1,8,7);
       } break;
  ...
  switch(cursore_menu)
   ...
   case 0://menu iniziale + potenza motore //prova
    ...
    if(tabella!=set.N_tabella_potenza) set.segnalazione_=2;
    cerca_potenza_nominale();
...
if(identificazione==2)
 {
 presenta_scritta((unsigned char *)&cambio_di_potenza,0,0,1,1,0,16);
 }
else if(identificazione<10)
 {
 asm
  {
  LDA identificazione
 ...
 presenta_scritta((unsigned char *)&lettura_allarmi,k,0,24,1,0,16);
 }
else
 {
 asm
...
void presenta_stato_motore(void)
{
int pressione, temperatura, flusso_medio;
if(toggle_func==0)//presenta le letture
 ...
 if(timer_aggiorna_misure==0)
  ...
  if(set.motore_on==1)
   ...
   if(mono_tri_fase==0) //con alimentazione monofase
    ...
    if(alternanza_presentazione==0x80)
     ...
     if(set.abilita_sensore_pressione)
      ...
      {
      presenta_energia(volume_decrementato,0,0,10,5,1);
      display[0][15]='L';
      }
     ...
     if(set.abilita_sensore_temperatura) presenta_unsigned(temperatura,0,1,12,3); else display[1][15]=' ';
     }
    ...
    if(alternanza_presentazione==0x21)
     ...
     {
     alternanza_presentazione=0x80;
     presenta_menu((unsigned char *)&dati_presentati_in_ON_trifase,0,2,0);
     }
    else if(alternanza_presentazione==0x41)
     {
     alternanza_presentazione=0x40;
     presenta_menu((unsigned char *)&dati_presentati_in_ON_trifase,0,2,2);
     }
    ...
    if(alternanza_presentazione==0x80)
     ...
     if(set.abilita_sensore_temperatura) presenta_unsigned(temperatura,0,0,0,3);
      else {presenta_unsigned(tensione_media,0,0,0,3); display[0][3]='V'; display[0][4]=' ';}
     if(potenza_media<0) potenza_media=0;
     presenta_unsigned(potenza_media,0,0,5,5);
     if(set.abilita_sensore_flusso)
      {
      presenta_unsigned(flusso_medio,0,0,11,4);
      display[0][15]='L';
      }
   ...
   {
   segnala_attendere=0;
   presenta_scritta((unsigned char *)&attendere,0,0,1,1,0,16);
   timer_aggiorna_misure=2000;
   }
  else
   {
...
if(set.motore_on)//condizioni per riavviamento
 ...
 if(relay_alimentazione==0)
  ...
  if(timer_avviamento_monofase==0)
   {
   if((set.timer_riavviamento==0)&&(set.timer_relay_avviamento==0))
    {
  ...
  if((remoto==0)||((set.modo_start_stop)&&(set.abilita_sensore_pressione)&&(pressione>set.pressione_spegnimento)))
   {
   set.timer_riavviamento=5;//attesa minima per riavviamento
   timer_avviamento_monofase=0;
   relay_alimentazione=0;
   if(remoto==0) set.segnalazione_=0;
   }
  } 
 }
...
if(toggle_func==0)//marcia normale
 ...
 if(salita_start)
  ...
  arresto_per_volume=0;
  timer_commuta_presentazione=0;
  if((set.timer_riavviamento==0)&&(timer_avviamento_monofase==0)&&(remoto==filtro_pulsanti))
   {
   if(set.timer_relay_avviamento==0)
    {
    if(set.motore_on==0) volume_decrementato=set.volume_massimo;
    set.motore_on=1;
    salvataggio_funzioni=1; 
    set.segnalazione_=1;
    timer_avviamento_monofase=durata_avviamento;
 ...
 else if((salita_stop)||(arresto_per_volume))
  {
  arresto_per_volume=0;
  salita_stop=0;
  if(relay_alimentazione) set.timer_riavviamento=5;//attesa minima per riavviamento
  else set.timer_riavviamento=0;
  timer_avviamento_monofase=0;
  relay_alimentazione=0;
  salita_start=0;
  salita_esc=0;
...
if((relay_alimentazione)&&(set.segnalazione_<20)) //alimentazione presente
 {
 ...
 if((set.abilita_sensore_pressione)&&(pressione<max_pressione_negativa)) //allarme sensore
  ...
  set.timer_riavviamento=set.timer_ritorno_da_emergenza_sensore;
  } 
  ...
  set.timer_riavviamento=attesa_ritorno_da_emergenza_I;
  } 
  ...
  set.timer_riavviamento=set.ritardo_riaccensione_mandata_chiusa;
  } 
 else
  {
  if((I1_rms>limitazione_I_avviamento)||(I2_rms>limitazione_I_avviamento)||(I3_rms>limitazione_I_avviamento)) //protezione per avviamento mancato
   ...
   if(tensione_media<sottotensione_consentita) //tensione insufficiente
    ...
    if(timer_attesa_intervento_tensione==0)
     ...
     set.timer_riavviamento=attesa_ritorno_da_emergenza_V;
     }
    } 
 ...
 if(set.segnalazione_<segnalazioni_senza_intervento)
  {
  if(set.segnalazione_>1) set.segnalazione_=1;
  if((set.abilita_sensore_pressione)&&(pressione>set.pressione_emergenza-5)) set.segnalazione_=19;//segnalazione pressione_emergenza
  else
   {
   if((set.abilita_sensore_temperatura>0)&&(temperatura>temperatura_segnalazione)) set.segnalazione_=15;//segnalazione con 5°C in anticipo
   else
    {
    if(I2xT>limitato_delta_T) set.segnalazione_=10;
    else
     {
     if((dissimmetria>dissimmetria_tollerata)&&(mono_tri_fase)) set.segnalazione_=18;
     else
      { 
      if((squilibrio>squilibrio_tollerato)&&(mono_tri_fase)) set.segnalazione_=17;
      else
       {
       if(tensione_media>sovratensione_consentita) set.segnalazione_=11;
       else
        {
        if(tensione_media<sottotensione_consentita) set.segnalazione_=12;
        else
         {
         if((set.abilita_sensore_flusso)&&(flusso_medio>set.limite_maximum_flow)) set.segnalazione_=16;
         else
          {
          if((set.abilita_sensore_pressione)
          &&(set.modo_start_stop==0)
          &&(potenza_media<potenza_mandata_chiusa)&&(pressione>set.pressione_spegnimento)) set.segnalazione_=13;
          else if((set.abilita_sensore_flusso)&&(flusso_medio<set.limite_minimum_flow)) set.segnalazione_=13;
          else
           {
           if(set.segnalazione_==13) set.segnalazione_=1;
           if(potenza_media<potenza_a_secco) set.segnalazione_=14; else set.segnalazione_=1;
           }
          }
         } 
        } 
...
presenta_scritta((char*)&scritta_numero_serie,0,0,0,1,0,9);
...
tentativi_avviamento=set.tentativi_avviamento_a_secco=N_tentativi_motore_bloccato;
precedente_segnalazione=set.segnalazione_;
inizializza_timer();
asm
 {
 LDA #$d3
